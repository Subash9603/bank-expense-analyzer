...
function renderCategoryFiltersHorizontal() {
    ...
    // Get unique categories from currently loaded user transactions
...
function renderCharts() {
    ...
    // Populate Top Spending Category breakdown details
    const statsList = document.getElementById('statsList');
    statsList.innerHTML = '';
    
    const totalExpenses = summary.total_expenses || 1; // avoid divide by zero
...
function evaluateAmountExpression(val) {
    // Strip everything except numbers, +, -, *, /, decimals, and parentheses
...
function addToAmount(num) {
    ...
    if (!curVal || curVal === '0') {
        ...
        // If it ends in a math operator, just append, otherwise add '+'
        if (/[\+\-\*\/]$/.test(curVal)) {
...
function triggerAICategorization() {
    ...
    .catch(err => console.error("AI categorization error:", err));
}
...
function simulateVoiceInput() {
    ...
    // Wave animation overlay simulating recording
    const descInput = document.getElementById('description');
...
function setupEventListeners() {
    ...
    document.getElementById('categoryGrid').addEventListener('click', (e) => {
        ...
        // Match payment method or transaction type if category dictates
    ...
    // Auto trigger AI Categorization on description blur
    document.getElementById('description').addEventListener('blur', () => {
        // Only run if amount is empty to prevent overwrites
    ...
    // Toggle between categories and trend stats graphs
    ...
    document.getElementById('expenseForm').addEventListener('submit', (e) => {
        ...
        .catch(err => console.error("Transaction save error:", err));
    });
}
